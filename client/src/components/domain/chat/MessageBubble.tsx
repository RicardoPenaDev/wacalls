...
type InteractiveData = {
  ...
  body: string;
  footer?: string;
  buttons?: Array<{ ID?: string; Title?: string; id?: string; title?: string; type?: string; Type?: string; url?: string; URL?: string }>;
  buttonText?: string;
  sections?: Array<{
      ...
      {data.variant === "buttons" && Array.isArray(data.buttons) && (
        <div className={`mt-2 flex flex-col gap-1 border-t pt-1 ${divider}`}>
          {data.buttons.map((b, i) => {
            ...
            const title = b.Title || b.title || b.ID || b.id || `Opção ${i + 1}`;
            if (url) {
              return (
                <a
                  key={i}
                  ...
                  className={`flex items-center justify-center gap-1.5 rounded-md px-2 py-1.5 text-center text-[13px] font-medium transition hover:opacity-90 ${
                    mine ? "bg-primary-foreground/20 text-primary-foreground underline" : "bg-primary/10 text-primary hover:bg-primary/15"
                  }`}
                >
                  <span>{title}</span>
