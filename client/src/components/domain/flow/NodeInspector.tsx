...
import { ExternalLink, Link, Plus, Trash2, TriangleAlert } from "lucide-react";
  ...
  return (
    <div className="space-y-2">
          ...
          <div key={i} className="space-y-1.5 rounded-lg border bg-muted/20 p-2">
            <div className="flex items-center gap-1.5">
              {suportaLink && (
                <select
                  ...
                  value={isUrl ? "url" : "reply"}
                  onChange={(e) => {
                    const nextType = e.target.value;
                    const copia = opcoes.map((opt, idx) => {
                      if (idx !== i) return opt;
                      if (nextType === "url") {
                        return { ...opt, type: "url", url: opt.url || "https://" };
                      }
                      const { url: _url, ...rest } = opt;
                      return { ...rest, type: "reply" };
                    });
                    onChange(copia);
                  }}
                  title="Tipo do botão"
                >
                  <option value="reply">Botão normal</option>
              ...
              {!isUrl && (
                <Input
                  ...
                  title="Identificador interno da opção"
                />
              )}
              <Input
                value={String(o.label ?? "")}
                onChange={(e) => alterar(i, "label", e.target.value)}
                placeholder={isUrl ? "Texto do link" : "O que o cliente vê"}
            ...
            {suportaLink && isUrl && (
              <div className="flex items-center gap-1.5 pt-0.5">
                <Link className="ml-1 h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                <Input
                  value={String(o.url ?? "")}
                  onChange={(e) => alterar(i, "url", e.target.value)}
                  placeholder="https://exemplo.com.br"
                  className="h-8 flex-1 font-mono text-xs text-primary"
                />
              </div>
            )}
          </div>
        ...
        {suportaLink && (
          <Button
            ...
            onClick={() =>
              onChange([
                ...opcoes,
                { key: `link_${opcoes.length + 1}`, label: "Acessar link", type: "url", url: "https://" },
              ])
            }
          >
      ...
      <p className="text-[11px] leading-snug text-muted-foreground">
        {suportaLink
          ? "Botões de link abrem o endereço no navegador do cliente ao clicar. Botões normais aguardam resposta e avançam no fluxo."
          : "Cada opção vira uma saída do nó — ligue cada uma ao próximo passo. O id identifica a escolha."}
      </p>
    </div>
  );
};
            ...
            {node.type !== "chat_input" && (
              <>
                ...
                <Campo label="Opções">
                  <EditorDeOpcoes
                    opcoes={opcoes}
                    suportaLink={node.type === "chat_msg_api" || node.type === "chat_menu"}
