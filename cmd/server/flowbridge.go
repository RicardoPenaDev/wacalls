...
// FlowButton is a single quick-reply or URL redirection button rendered natively by WhatsApp.
...
// SendWhatsAppButtons sends an InteractiveMessage with native quick-reply or
// cta_url buttons. WhatsApp mobile renders these as real tappable buttons.
func (b *FlowBridge) SendWhatsAppButtons(ctx context.Context, sessionID, to, body, footer string, buttons []FlowButton) error {
	...
	for i, bt := range buttons {
		...
		if (bt.Type == "url" || bt.URL != "") && bt.URL != "" {
			targetURL := strings.TrimSpace(bt.URL)
			if !strings.HasPrefix(targetURL, "http://") && !strings.HasPrefix(targetURL, "https://") {
				targetURL = "https://" + targetURL
			}
			params := fmt.Sprintf(`{"display_text":%q,"url":%q,"merchant_url":%q}`, bt.Title, targetURL, targetURL)
	...
	if err != nil {
		...
		for i, bt := range buttons {
			if bt.URL != "" {
				...
				lines = append(lines, fmt.Sprintf("[ %s ]: %s", bt.Title, targetURL))
			} else {
