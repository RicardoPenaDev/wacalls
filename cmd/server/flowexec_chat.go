	...
	case "chat_menu", "chat_msg_api", "chat_input":
		...
		if opts, ok := n.Data["options"].([]interface{}); ok {
			for i, raw := range opts {
				...
				btnType, _ := o["type"].(string)
				btnURL, _ := o["url"].(string)
				if btnURL != "" {
					btnURL = renderTemplate(btnURL, vars)
				}
				buttons = append(buttons, FlowButton{
					ID:    id,
					Title: renderTemplate(label, vars),
					Type:  btnType,
					URL:   btnURL,
				})
			}
		...
		default:
			...
			for i, bt := range buttons {
				if bt.URL != "" {
					lines = append(lines, fmt.Sprintf("[ %s ]: %s", bt.Title, bt.URL))
				} else {
		...
		if !hasReplyButtons && len(sections) == 0 && len(buttons) > 0 {
			// All buttons are URL redirection links; WhatsApp will not send an inbound reply.
			// Continue immediately to the next node if connected.
			time.Sleep(900 * time.Millisecond)
			return "", "", nil
		}
